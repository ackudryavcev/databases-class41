...
async function transfer(fromAccount, toAccount, amount, remark) {
    ...
    try {
        await session.withTransaction(async() => {
            ...
            const answerFrom = await coll1.updateOne(fromAccountQuery, fromAccountUpdate, { session });
            ...
            const answerTo = await coll1.updateOne(toAccountQuery, toAccountUpdate, { session });
            if (answerTo.acknowledged && answerFrom.acknowledged) console.log('Transfer was successful');
